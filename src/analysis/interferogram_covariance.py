...
def cov(input, output, threads, config, params, wildcards):
    ...
    cov_ax_min.matshow(np.abs(outer_coh[min_var_idx]),vmin=0, vmax=1)
    cov_ax_max =  f.add_subplot(gs[2::,2::4])
    cov_ax_max_mappable = cov_ax_max.matshow(np.abs(outer_coh[max_var_idx]),vmin=0, vmax=1)
    #Add colorbar
    cbar_ax = f.add_subplot(gs[::,-1])
    cbar = f.colorbar(cov_ax_max_mappable, cax=cbar_ax)
    cbar.set_label('Correlation coefficent')
    #Set ticks
    ticks_positions = range(20)
    ...
    mean_ax.annotate('',xy=(max_var_idx), xytext=(0,0),xycoords=mean_ax.transData, textcoords=cov_ax_max.transData, arrowprops=arrowprops)
